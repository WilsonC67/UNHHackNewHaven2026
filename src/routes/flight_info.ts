...
function mapItinerary(it: Travel.SerpApiItinerary): Travel.FlightOffer {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const data = await response.json() as Travel.SerpApiResponse
        ...
        const flights: Travel.FlightOffer[] = itineraries.slice(0, 5).map(mapItinerary)
...
router.post('/', (req, res) => {
    ...
    const result: Travel.ResolvedAirport = {
        iata,
        name: query,
        city: query,
    }
